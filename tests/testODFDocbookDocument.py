...
class TestODFDocbookDocument(CPSOOoTestCase.CPSOOoTestCase):
    ...
    def afterSetUp(self):
        ...
        f = open(join(abspath(dirname(__file__)), 'input', 'test.odt'), 'rb')
        self.file = File('test', 'test.odt', f)
        self.file.content_type = 'application/vnd.oasis.opendocument.text'
        f.close()

    def beforeTearDown(self):
        self.logout()

    def testAddODFDocbookDocument(self):
        """Test creation of ODFDocbookDocument instance in root of workspaces.
        """
    ...
    def testRemoveODFDocbookDocument(self):
        """Test removal of ODFDocbookDocument instance in root of workspaces.
        """
        proxy, doc = self._createODFDocbookDoc()
        self.assertEqual(len([o for o in self.ws.contentValues()
                              if o.getContent().meta_type == self.doc_type]), 1)
    ...
    def testEditODFDocbookDocument(self):
        proxy, doc = self._createODFDocbookDoc()
        props = {
            'Title' : 'The title',
        ...
        self.assertEqual(doc.title, "Dix raisons d'utiliser OpenOffice.org",
                                    "Title is '%s' and should be 'Dix raisons d'utiliser OpenOffice.org'" % str(doc))
    ...
    def testExportXmlDocbook(self):
        ...
        arch_path = join(abspath(dirname(__file__)), 'output','test.odf.zip')
...
def test_suite():
    ...
    suite.addTest(unittest.makeSuite(TestODFDocbookDocument))
    return suite
